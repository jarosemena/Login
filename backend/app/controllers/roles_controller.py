...
@user_controller.route('/rolesuser/user/<int:user_id>', methods=['GET'])
def get_role_by_user_id(user_id):
    role = RolesUser.query.filter_by(user_id=user_id).first_or_404()
    return jsonify(role.to_dict())

@user_controller.route('/rolesuser/user/<int:user_id>/role', methods=['POST'])
...
@user_controller.route('/rolesuser/user/<int:user_id>/roles', methods=['DELETE'])
...
@user_controller.route('/rolesuser/user/<int:user_id>/role/<int:role_id>', methods=['DELETE'])
